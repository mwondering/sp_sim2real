...
struct ProgramOptions {
  ...
  std::optional<int> state_port;
  std::optional<int> cmd_port;
};
...
ProgramOptions parse_options(int argc, char ** argv)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--state-port" && i + 1 < argc) {
      options.state_port = std::stoi(argv[++i]);
      continue;
    }
    if (arg.rfind("--state-port=", 0) == 0) {
      options.state_port = std::stoi(arg.substr(std::string("--state-port=").size()));
      continue;
    }
    ...
    if (arg == "--cmd-port" && i + 1 < argc) {
      options.cmd_port = std::stoi(argv[++i]);
      continue;
    }
    if (arg.rfind("--cmd-port=", 0) == 0) {
      options.cmd_port = std::stoi(arg.substr(std::string("--cmd-port=").size()));
      continue;
    }
    ...
    if (arg == "-h" || arg == "--help") {
      std::cout
          << "Usage: g1_udp_bridge [--net IFACE] [--config PATH] "
             "[--state-host IP] [--state-port PORT] "
             "[--cmd-bind-host IP] [--cmd-port PORT] "
             "[--cmd-allowed-host IP]"
          << std::endl;
      std::exit(0);
    }
...
int main(int argc, char ** argv)
{
  ...
  try {
    ...
    if (options.state_port) {
      g1_bridge::validate_udp_port(*options.state_port, "--state-port");
      config.udp.state_port = *options.state_port;
    }
    if (options.cmd_port) {
      g1_bridge::validate_udp_port(*options.cmd_port, "--cmd-port");
      config.udp.cmd_port = *options.cmd_port;
    }
