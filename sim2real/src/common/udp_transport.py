...
class UDPRobotHighConfig:
    ...
    def from_data(cls, data) -> "UDPRobotHighConfig":
        return cls(
            state_bind_host=os.environ.get(
                "G1_STATE_BIND_HOST", str(_cfg_get(data, "state_bind_host"))
            ),
            state_port=int(os.environ.get("G1_STATE_PORT", _cfg_get(data, "state_port"))),
            cmd_host=os.environ.get("G1_CMD_HOST", str(_cfg_get(data, "cmd_host"))),
            cmd_port=int(os.environ.get("G1_CMD_PORT", _cfg_get(data, "cmd_port"))),
        )
...
class UDPRobotLowConfig:
    ...
    def from_data(cls, data) -> "UDPRobotLowConfig":
        return cls(
            state_host=os.environ.get("G1_STATE_HOST", str(_cfg_get(data, "state_host"))),
            state_port=int(os.environ.get("G1_STATE_PORT", _cfg_get(data, "state_port"))),
            cmd_bind_host=os.environ.get(
                "G1_CMD_BIND_HOST", str(_cfg_get(data, "cmd_bind_host"))
            ),
            cmd_port=int(os.environ.get("G1_CMD_PORT", _cfg_get(data, "cmd_port"))),
        )
