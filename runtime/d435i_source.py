...
class D435iSource:
    ...
    def _resolve_worker_python(self) -> str:
        ...
        for candidate in candidates:
            ...
            try:
                probe = subprocess.run(
                    [
                        str(path),
                        "-c",
                        "import numpy; import pyrealsense2 as rs; "
                        "assert hasattr(rs, 'pipeline')",
                    ],
            ...
            if probe.returncode == 0:
                return str(path)
            error_lines = probe.stderr.strip().splitlines()
            detail = error_lines[-1] if error_lines else "import failed"
    ...
    def _apply_worker_metadata(self, metadata: object) -> None:
        ...
        if str(metadata.get("pixel_format", "")).lower() != "z16":
            raise RuntimeError("D435i worker pixel format must be z16")
        actual_profile = (
    ...
    def capture(self) -> D435iFrame | None:
        ...
        depth_raw = np.asarray(frame.get_data(), dtype=np.uint16).copy()
        return D435iFrame(
            depth_raw=depth_raw, capture_monotonic=capture_time
        )
    ...
    def _capture_worker(self) -> D435iFrame | None:
        ...
        if not payload:
            return None
        depth_raw = np.load(BytesIO(payload), allow_pickle=False)
        if depth_raw.dtype != np.uint16:
            raise RuntimeError(
                f"D435i worker frame dtype {depth_raw.dtype} != uint16"
            )
        if depth_raw.shape != (self.height, self.width):
            raise RuntimeError(
                "D435i worker frame shape "
                f"{depth_raw.shape} != {(self.height, self.width)}"
            )
        return D435iFrame(
